...
/* A roster entry is a name, or `{ name, personId }` once the desk
   assigns by picking. The engine reads the same two shapes; these are
   the desk's side of it, so nothing here has to care which it holds. */
const opName = (o) => (typeof o === "string" ? o : (o && o.name) || "");
...
/* Who is on the floor, as the last push said.

   This used to be reconstructed: the desk read twelve payloads back,
   rebuilt a roster from `operator.name`, redrew the schedule from it and
   compared the result turn by turn against what the floor was running,
   adopting it only on agreement. That was the right fix for a compiled-in
   file going stale - a correction made on one desk never reached
   another - but its reason to exist was that there was no list of people
   anywhere. There is one now, and the roster rides on the push itself,
   in the same row and transaction as the payloads it produced, so the
   two cannot disagree and there is nothing left to prove.

   The floor is never adopted over an edit already made on this screen.
   Arriving late and overwriting a half-typed name is the same silent
   clobber the whole mechanism exists to stop, only faster. */
async function adoptFloorRoster() {
  if (planTouched) return;
  try {
    const r = await api("/api/roster");
    if (!r.ok) return;                       // a service too old to know this route
    const body = await r.json();
    if (!Array.isArray(body.roster) || !body.roster.length) return;   // nothing pushed yet
    GROUPS = body.roster;
    remountRosters();
    renderPlan();
  } catch (e) {
    /* No roster to read is not a failure of this screen. */
  }
}
...
function renderGrid(p, kind) {
  ...
  p.groups.forEach((g, gi) => {
    ...
    const rows = (kind === "ops" ? g.ops.map(opName) : g.rigs);
    rows.forEach((name, i) => {
      const tr = el("tr");
      tr.style.setProperty("--gc", "var(--g" + gi + ")");
      ...
      for (let b = 0; b < p.nBlocks; b++) {
        ...
        if (kind === "ops") {
          ...
          td = oi === -1 ? cellNode("idle" + onHour, "-")
                         : cellNode("work name" + onHour, shortName(opName(g.ops[oi])));
        }
...
async function pushFloor() {
  ...
  try {
    const res = await api("/api/push", {
      method: "POST",
      headers: { "content-type": "application/json" },
      /* The roster these payloads were built from rides with them, so
         the next desk to open reads it back instead of reconstructing
         it - which is what lets a correction made here reach every
         other desk. */
      body: JSON.stringify({ payloads, roster: GROUPS }),
    });
