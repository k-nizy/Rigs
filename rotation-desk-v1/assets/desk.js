...
/* A roster entry is a name, or `{ name, personId }` once the desk
   ...
   `opKey` is for comparing two rosters. Two people with one name are
   two entries, and a comparison on the name alone would call them
   equal - which is the whole failure the person id exists to stop. */
...
function rosterFromFloor(payloads) {
  ...
  for (const [key, rigs] of byGroup) {
    ...
    /* Keyed by person, not by name. A desk that rebuilt the roster from
       names alone would read the floor back and push it out again with
       every `personId` gone - silently, off a screen nobody touched, and
       the id is the one thing a take is filed under that nothing
       downstream could tell had gone missing. */
    const entry = (o) => (o.personId ? { name: o.name, personId: o.personId } : o.name);
    const everyone = new Map();
    rigs.forEach(p => p.turns.forEach(t => {
      const o = t.operator;
      if (o && o.name) everyone.set(opKey(entry(o)), entry(o));
    }));
    ...
    const onRig = opening.map(t => entry(t.operator));
    const onKeys = onRig.map(opKey);
    if (new Set(onKeys).size !== onKeys.length) return null;   // one person, two rigs

    const held = new Set(onKeys);
    const off = [...everyone.values()].filter(e => !held.has(opKey(e)));
    out.push({
      key: key,
      task: rigs[0].task,
  ...
  return payloads.every(want => {
    ...
    return got.turns.every((t, i) => {
      ...
      return t.from === w.from && t.to === w.to && t.minutes === w.minutes
          ...
          && (t.operator.personId || null) === ((w.operator && w.operator.personId) || null)
...
const sameRoster = (a, b) =>
  a.length === b.length && a.every((g, i) =>
    ...
    g.rigs.join() === b[i].rigs.join() &&
    g.ops.map(opKey).join() === b[i].ops.map(opKey).join());
