...
function envelope(event, bucket, data, who) {
  ...
  return {
    ...
    /* The person, when the sheet names one. Absent otherwise - not
       null - so a floor with no people table files byte for byte what
       it always did. Read from `c`, the operator captured when the
       thing began, for the same reason the seat and the name are: a
       take that runs past a turn boundary belongs to whoever pressed
       start, not to whoever the clock says is on now. */
    ...(c && c.turn.operator.personId ? { personId: c.turn.operator.personId } : {}),
    bucket: bucket,
    event: event,
    data: data || {},
  };
...
function emit(event, bucket, detail, data, who) {
  ...
  /* And the running total the events do not carry, at the same moment
...
const memoryJournal = {
  durable: false,
  async load() { return { events: [], videos: [], stint: null }; },
  async appendEvent() {},
  async forgetEvents() {},
  async putVideo() {},
  async forgetVideo() {},
...
function browserJournal() {
  ...
  function db() {
    ...
    opening = new Promise((resolve, reject) => {
      ...
      req.onupgradeneeded = () => {
        ...
        /* One row per rig, not one per event: this is the running total,
           not a thing that happened. Keyed on rigId so writing it again
           replaces it rather than accumulating a history nobody reads. */
        if (!d.objectStoreNames.contains("stint")) {
          d.createObjectStore("stint", { keyPath: "rigId" });
        }
      };
  ...
  return {
    ...
    async load(rigId) {
      ...
      const stint = (await tx("stint", "readonly", (s) => s.get(rigId))) || null;
      return { events, videos, stint };
    },
    ...
    putStint(row) { return tx("stint", "readwrite", (s) => s.put(row)); },
  };
}
...
async function recoverJournal() {
  ...
  return { events: events.length, videos: videos.length,
           stint: (held && held.stint) || null };
}

/* The part of a stint that is not an event.
 *
 * Every episode is filed and journalled, so the *events* survive a
 * reload. What does not is `S` - `boot()` rebuilds it from zero, because
 * every boot is treated as the start of a shift. The operator who
 * reloads therefore watches their own episode count and recorded seconds
 * go back to nothing, while the desk, which reads the ledger, still has
 * them. This is the running total, saved beside the events it came from.
 *
 * Stamped with the shift and the turn it belongs to, and resumed only
 * when all three match. A turn label is "HH:MM" and repeats on every
 * shift of every day, so the turn alone would let a rig booting a day
 * later inherit a stranger's numbers - the same trap `floor.py` names
 * when it matches a finished block against a schedule.
 *
 * Elapsed rather than `stintAt`, because `S.t` restarts at zero on a
 * boot and an absolute mark from before it would read as negative time
 * on the rig. Elapsed is the fact; the mark is derived from it.
 */
function stintRecord() {
  return {
    rigId: RIG_ID,
    shiftDate: PAYLOAD ? PAYLOAD.shift.date : null,
    shiftLabel: PAYLOAD ? PAYLOAD.shift.label : null,
    ...
    stint: S.stint,
    episode: S.episode,
    recordedSecs: S.recordedSecs,
    faultSecs: S.faultSecs,
    downSecs: S.downSecs,
    elapsedSecs: S.t - S.stintAt,
  };
}

/* Written on every event, which is the same cadence the events
   themselves are journalled at and needs no timer of its own. The
   counters that move between events are fault and downtime seconds; a
   crash mid-outage loses at most the seconds since it began, and it
   loses them in the direction that charges the operator rather than
   credits them. */
function checkpointStint() {
  if (!S || !journal.putStint) return;
  onJournal(() => journal.putStint(stintRecord()), "the stint so far");
}

/* Same turn, same shift, same rig - or start clean.
   Deliberately not a grace period. A turn is what work is attributed to,
   so the turn boundary is the honest edge: a rig that was off long
   enough to matter has crossed one, and the crew change crosses one by
   definition. Nothing here needs a number chosen by hand. */
function resumeStint(held) {
  if (!held || !S || !PAYLOAD || !turnKey) return false;
  if (held.turnKey !== turnKey) return false;
  if (held.shiftDate !== PAYLOAD.shift.date) return false;
  if (held.shiftLabel !== PAYLOAD.shift.label) return false;

  S.stint = held.stint || 0;
  S.episode = held.episode || 0;
  S.recordedSecs = held.recordedSecs || 0;
  S.faultSecs = held.faultSecs || 0;
  S.downSecs = held.downSecs || 0;
  S.stintAt = S.t - (held.elapsedSecs || 0);
  /* The checklist is not restored, and that is deliberate. The numbers
     are a record of what happened; the check is a statement about the
     rig right now, and a machine that just restarted has not been
     looked at since. */
  return true;
}
...
/* What the stint has accumulated, the same way rigVideo and rigOutbox
   report theirs - so a test can ask whether a reload kept it. */
window.rigStint = () => (S ? {
  stint: S.stint, episode: S.episode,
  recordedSecs: Math.round(S.recordedSecs),
  faultSecs: Math.round(S.faultSecs), downSecs: Math.round(S.downSecs),
  assignedSecs: Math.round(assignedSecs()),
} : null);
...
async function flush() {
  ...
  /* A rig only ever uploads its own events.

     `switchRig` is a demo affordance and a real rig is only ever itself,
     but the outbox is module-level and `boot()` does not clear it, so
     events still owed when the rig switches stay queued under the next
     identity. One batch then carries two, and the service refuses a
     batch naming another rig *whole* - which is right, and which took
     this rig's own work down with the foreign events.

     Parked, not dropped. The journal is scoped by rig on load, so
     switching back finds them again; dropping them here would lose
     work the screen has already promised is queued. */
  for (let i = outbox.length - 1; i >= 0; i--) {
    if (outbox[i].rigId !== RIG_ID) outbox.splice(i, 1);
  }
...
async function start(rigId) {
  ...
  /* After boot, because boot builds S from zero and reads the turn in
     progress; both have to have happened before the held stint can be
     matched against them. Written straight back, so the row on disk
     describes the stint that is actually running rather than the zeros
     boot just checkpointed. */
  const resumed = resumeStint(owed && owed.stint);
  if (resumed) checkpointStint();
  ...
  if (resumed) {
    emitLog("stint_resumed", "sessions",
            "picked the turn back up: " + S.episode + " episodes and " +
            clock(S.recordedSecs) + " recorded before the restart");
  }
  startUploader();
  startVideoUploader();
  startResync();
